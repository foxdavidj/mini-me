...
const {values}=parseArgs({args:Bun.argv.slice(3),options:{key:{type:'string'},email:{type:'string'},query:{type:'string'},file:{type:'string'}}});
...
const decisionSchema=z.object({items:z.array(z.object({key:z.string(),action:z.enum(['archive','question','keep']),reason:z.string().min(1),question:z.string().optional()}).strict()).max(1000)}).strict();
try{
 if(command==='ingest'){
  const counts=[];
  ...
  const message=await reader(item.email).message(item.id);privateWrite(pathFor(item.key),JSON.stringify({email:item.email,...message},null,2));console.log(JSON.stringify({email:item.email,...message}));
  ...
  for(const decision of plan.items){if(!items.some(x=>x.key===decision.key))throw Error('Unknown message');if(decision.action==='question'&&!decision.question?.trim())throw Error('A question needs question text');}
  ...
  for(const decision of plan.items){
   const item=items.find(x=>x.key===decision.key)!;if(!['pending','answered','following_up'].includes(item.status))continue;
  ...
  console.log(JSON.stringify({archived:results.filter(x=>x.ok).length,failed:results.filter(x=>!x.ok)}));
  if(results.some(x=>!x.ok))process.exitCode=1;
 }else if(command==='status')console.log(JSON.stringify(reviews.snapshot()));
 else throw Error('Use ingest, read --key, search --email --query, apply --file, or status');
