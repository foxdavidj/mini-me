...
const decisionSchema=z.object({items:z.array(z.object({key:z.string(),action:z.enum(['archive','question','keep','label']),reason:z.string().min(1),question:z.string().optional(),labels:z.array(assistantLabelName).min(1).max(8).optional(),complete:z.boolean().optional()}).strict()).max(1000)}).strict();
try{
 if(command==='ingest'){
  const counts=[];
  ...
  for(const decision of plan.items){
   ...
   if(decision.labels){const result=await tagMail(reviews,item,decision.labels,reader(item.email));tagged.push(result);if(!result.ok)continue;}
 ...
 else throw Error('Use ingest, read --key, search --email --query, labels --email, apply --file, or status');
